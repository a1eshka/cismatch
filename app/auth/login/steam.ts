...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        ...
        console.log(`Redirecting to Steam: ${steamAuthUrl}?openid.claimed_id=http://specs.openid.net/auth/2.0/identifier_select&openid.mode=checkid_setup&openid.return_to=${encodeURIComponent(redirectUri)}`);
