    ...
    <div>
      ...
      <div>
        ...
        {sortedComments.map(comment => (
            ...
            <AvatarImage  src={comment.author.steam_avatar ? comment.author.steam_avatar : comment.author.avatar_url} alt={comment.author.name}/>
          </Avatar> 
          <div className='ml-2 text-gray-500'>{comment.author.name} {isToday(comment.formatted_date) ? 'cегодня' : comment.formatted_date} в {comment.formatted_time}: </div>
          </div>
          <div className='bg-gray-400/25 rounded-lg my-3 p-3' key={comment.id}> {comment.text}</div>
          </>
        ))}
      </div>
    </div>
